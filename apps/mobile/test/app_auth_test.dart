...
final testDevice = <String, dynamic>{
...
void main() {
  ...
  test(
    'Concurrent requests share one refresh and store only refresh credentials',
    () async {
      var calls = 0;
      final auth = AppAuth(
        deviceProvider: () async => testDevice,
        product: 'test',
      ...
      expect(await Future.wait([auth.accessToken(), auth.accessToken()]), [
        'access',
        'access',
      ]);
      expect(calls, 1);
  ...
  test(
    'Invalid refresh clears session but transport failure preserves it',
    () async {
      final auth = AppAuth(
        deviceProvider: () async => testDevice,
        product: 'test',
  ...
  test(
    'Registration response cannot become an authenticated session',
    () async {
      final auth = AppAuth(
        deviceProvider: () async => testDevice,
        product: 'test',
      ...
      await expectLater(
        auth.save({'message': 'Confirm your email'}),
  ...
  test(
    'Restart restores refresh session and logout revokes before clearing',
    () async {
